...
async def test_admin_qqs_empty_allows_any_user_for_every_subcommand(
    ...
    monkeypatch.setattr(
        instance,
        "_build_configured_next_dungeon",
        lambda *, today: "【杖剑助手 · 下一个副本】",
    )
    monkeypatch.setattr(
        instance,
        "_build_configured_progress",
        lambda *, today: "【杖剑助手 · 服务器进度】",
    )
    monkeypatch.setattr(
        instance,
    ...
    assert await instance.handle_zjcs_command(
        stream_id="s", user_id="第四个用户", matched_groups={"sub": "副本"}
    ) == (True, "下一个副本已发送", True)
    assert await instance.handle_zjcs_command(
        stream_id="s", user_id="第五个用户", matched_groups={"sub": "进度"}
    ) == (True, "服务器进度已发送", True)
    assert await instance.handle_zjcs_command(
...
@pytest.mark.asyncio
async def test_next_dungeon_command_uses_real_timeline_and_ignores_remind_day(
    tmp_path, monkeypatch
) -> None:
    class _FixedDatetime:
        @staticmethod
    ...
    config["reminders"]["dungeon_remind_day"] = 0
    ...
    result = await instance.handle_zjcs_command(
        ...
        group_id="123456",
        platform="qq",
        matched_groups={"sub": "副本"},
    )

    assert result == (True, "下一个副本已发送", True)
    assert len(instance.ctx.send.calls) == 1
    message, stream_id, _ = instance.ctx.send.calls[0]
    assert stream_id == "operator-stream"
    assert "【杖剑助手 · 下一个副本】" in message
    assert "龙之国 · 仙海云舟" in message
    assert "开放日期：2026-09-25" in message
    assert "距离开放：11 天后" in message
    assert "普通：1650万" in message
    assert "准备建议：2026-09-24 开始攒副本次数（10 天后）。" in message
...
@pytest.mark.asyncio
async def test_next_dungeon_alias_roots_share_one_implementation(
    tmp_path, monkeypatch
) -> None:
    instance = ZjcsGuildNotifier()
    instance._ctx = make_context(tmp_path)
    ...
    first = await instance.handle_zjcs_command(
        stream_id="s1", matched_groups={"sub": "副本"}
    )
    second = await instance.handle_zjcs_command(
        stream_id="s2", matched_groups={"sub": "副本"}
    )

    assert first == second == (True, "下一个副本已发送", True)
    assert [call[1] for call in instance.ctx.send.calls] == ["s1", "s2"]
    assert not (tmp_path / "notification_state.json").exists()
...
@pytest.mark.asyncio
async def test_progress_command_uses_real_timeline_and_ignores_remind_day(
    tmp_path, monkeypatch
) -> None:
    class _FixedDatetime:
        @staticmethod
    ...
    result = await instance.handle_zjcs_command(
        ...
        group_id="123456",
        platform="qq",
        matched_groups={"sub": "进度"},
    )

    assert result == (True, "服务器进度已发送", True)
    assert len(instance.ctx.send.calls) == 1
    message, stream_id, _ = instance.ctx.send.calls[0]
    assert stream_id == "operator-stream"
    assert "【杖剑助手 · 服务器进度】" in message
    assert "当前服务器进度：开服第 88 天" in message
    assert "当前赛季：S2" in message
    assert "当前赛季进度：第 42 天" in message
    assert "龙国第二期遗物：道衍天机" in message
    assert "新幻兽：竹林仙君" in message
    assert "非人哉联动" not in message
    # 只读查询：不解析目标群、不广播、不写通知状态。
    assert instance.ctx.chat.group_stream_calls == []
    assert instance.ctx.chat.calls == []
    assert not (tmp_path / "notification_state.json").exists()
...
@pytest.mark.asyncio
async def test_progress_alias_roots_share_one_implementation(
    tmp_path, monkeypatch
) -> None:
    instance = ZjcsGuildNotifier()
    instance._ctx = make_context(tmp_path)
    ...
    first = await instance.handle_zjcs_command(
        stream_id="s1", matched_groups={"sub": "进度"}
    )
    second = await instance.handle_zjcs_command(
        stream_id="s2", matched_groups={"sub": "进度"}
    )

    assert first == second == (True, "服务器进度已发送", True)
    assert [call[1] for call in instance.ctx.send.calls] == ["s1", "s2"]
    assert not (tmp_path / "notification_state.json").exists()


@pytest.mark.asyncio
async def test_secret_treasure_command_uses_real_timeline_and_ignores_remind_day(
    tmp_path, monkeypatch
) -> None:
    class _FixedDatetime:
        @staticmethod
    ...
    config = make_config()
    config["server"]["open_date"] = "2026-06-19"
    config["reminders"]["secret_treasure_remind_day"] = 0
    ...
    result = await instance.handle_zjcs_command(
        ...
        group_id="123456",
        platform="qq",
        matched_groups={"sub": "秘宝"},
    )

    assert result == (True, "秘宝查询已发送", True)
    assert len(instance.ctx.send.calls) == 1
    message, stream_id, _ = instance.ctx.send.calls[0]
    assert stream_id == "operator-stream"
    assert "【杖剑助手 · 秘宝大作战】" in message
    assert "秘宝大作战·第12期" in message
    assert "重点奖励：原初宝石" in message
    assert "秘宝大作战·第13期" in message
    assert "重点奖励：自选奇迹遗物箱·龙Ⅱ（道衍天机）" in message
    # 只读查询：不解析目标群、不广播、不写通知状态。
    assert instance.ctx.chat.group_stream_calls == []
    assert instance.ctx.chat.calls == []
    assert not (tmp_path / "notification_state.json").exists()
...
@pytest.mark.asyncio
async def test_secret_treasure_alias_roots_share_one_implementation(
    tmp_path, monkeypatch
) -> None:
    instance = ZjcsGuildNotifier()
    instance._ctx = make_context(tmp_path)
    ...
    first = await instance.handle_zjcs_command(
        stream_id="s1", matched_groups={"sub": "秘宝"}
    )
    second = await instance.handle_zjcs_command(
        stream_id="s2", matched_groups={"sub": "秘宝"}
    )
