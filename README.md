...
当前插件显示名称为「杖剑助手」，插件版本为 `0.1.2`，配置版本为 `1.2.0`。

- 支持配置多个目标 QQ 群（`target.group_ids`），旧的单群 `group_id` 配置会自动迁移；
- 同一次每日检查命中的提醒会按日期和类型整理成一条合并消息，向每个需要的群各发送一条；
- 已发送去重状态按群独立保存（notification state V2）：一个群发送失败不影响其他群，重启后只对未记录的群重试；
- 副本、秘宝大作战、宾果抽抽乐、幸运刮刮乐、菲涅克的谜题及重要事件各使用一个提前提醒天数（整数），填写 `0` 关闭该类别提醒；旧的列表配置会自动迁移为取最大有效正整数；
- S4、S5、S6 赛季开始日期使用独立字段，留空表示尚未配置；
- operator 可用 `/zjcs_preview` 查看今日理论提醒，该预览不读取或写入业务去重状态；
- operator 可用 `/zjcs_test` 向所有已配置目标群各发送一条明确标注的链路测试消息，逐群独立重试，该测试不写入业务去重状态。
