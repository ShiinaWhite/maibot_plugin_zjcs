...
当前插件版本为 `0.1.1`，配置版本为 `1.1.0`。

- 同一次每日检查命中的提醒会按日期和类型整理成一条合并消息；
- 副本、秘宝大作战、宾果抽抽乐、幸运刮刮乐、菲涅克的谜题及重要事件可分别设置提前提醒天数；
- S4、S5、S6 赛季开始日期使用独立字段，留空表示尚未配置；
- operator 可用 `/zjcs_preview` 查看今日理论提醒，该预览不读取或写入业务去重状态；
- operator 可用 `/zjcs_test` 向配置的目标群发送一条明确标注的链路测试消息，该测试不写入业务去重状态。
