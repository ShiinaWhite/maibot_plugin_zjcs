...
`杖剑助手` 是一个运行在 [MaiBot](https://github.com/MaiM-with-u/MaiBot) 上的插件（plugin id：`zjcs.guild-notifier`，当前版本 `0.1.2`），基于 MaiBot Plugin SDK 实现。它根据本地维护的《杖剑传说》时间线数据，每天定时检查服务器进度，提前向公会 QQ 群发送副本、活动和重要事件通知。
...
- **合并通知**：同一天命中的多个提醒合并为一条消息发给每个群，同一天内同一群不会收到多条刷屏消息；
...
- **`/zjcs_preview`**（operator）：预览今天按当前配置会生成的提醒，不读取也不写入通知状态；
- **`/zjcs_test`**（operator）：向所有已配置目标群各发送一条链路测试消息，逐群独立重试，不写入业务状态；
